...
-- 게시판 목록 삽입
INSERT INTO BOARD(BOARD_NUMBER,BOARD_NAME) VALUES (1,'만족해요');
INSERT INTO BOARD(BOARD_NUMBER,BOARD_NAME) VALUES (2,'불만 있어요');
INSERT INTO BOARD(BOARD_NUMBER,BOARD_NAME) VALUES (3,'건의 사항');

-- 고객이 선택한 게시판의 목록 조회
SELECT TITLE FROM WRITE WHERE BOARD_NAME = (SELECT BOARD_NAME FROM BOARD WHERE BOARD_NUMBER = ?)
...
-- 게시글 삽입
INSERT INTO WRITE(BOARD_NAME,WRT_NUM,TITLE,CONTENTS,WRT_DATE,USER_ID) 
VALUES ('만족해요',WRT_NUM_SEQ.NEXTVAL,'정말 추천','딱 맞는 상품 받았어요',TO_CHAR(SYSDATE),'zzangu11');
INSERT INTO WRITE(BOARD_NAME,WRT_NUM,TITLE,CONTENTS,WRT_DATE,USER_ID) 
VALUES ('불만 있어요' ,WRT_NUM_SEQ.NEXTVAL,'마음에 드는 상품이 없어요','선택지가 적네요..',TO_CHAR(SYSDATE),'ccccsu89');

-- 게시글 조회
SELECT * FROM WRITE;

-- 게시글 입력받으면 삽입 & 확인
INSERT INTO WRITE(BOARD_NAME,WRT_NUM,TITLE,CONTENTS,WRT_DATE,USER_ID) VALUES (?,WRT_NUM_SEQ.NEXTVAL,?,?,TO_CHAR(SYSDATE),?);
SELECT * FROM WRITE;
...
-- 댓글 입력받으면 삽입 & 확인
INSERT INTO REPLY(WRT_NUM,COMMENT_NUM,COMMENT_CON,USER_ID) VALUES (WRT_NUM_SEQ.NEXTVAL,COMMENT_NUM_SEQ.NEXTVAL,?,?);
SELECT * FROM REPLY;
...
-- 고객 정보 입력받으면 삽입
INSERT INTO CUSTOMER VALUES (USER_NUMBER.NEXTVAL,?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?);
...
-- 고객이 상품을 선택하면 대출 실행 번호 매기기..?
INSERT INTO LOAN_PRODUCT VALUES (LOAN_NUMBER.NEXTVAL, ?);

-- 고객이 대출 기간 설정하면 대출 정보 테이블에 넣기
INSERT INTO LOAN_INFO (PERIOD) VALUES (?);

-- 고객 정보 끌어와서 고객 대출 정보 테이블에 넣기
INSERT INTO LOAN_INFO (USER_NUMBER, NAME) VALUES (USER_NUMBER, NAME);

-- 대출 상품 정보 끌어와서 고객 대출 정보 테이블에 넣기
INSERT INTO LOAN_INFO (LOAN_NUMBER, PRODUCT_NUMBER, PRODUCT_NAME, RATE, MAX_LOAN, MAX_MONTH) 
VALUES (LOAN_NUMBER, PRODUCT_NUMBER, PRODUCT_NAME, RATE, MAX_LOAN, MAX_MONTH);
...
-- 직업, 신용, 나이 3가지 조회 (나이 조회는 자바에서 구현 예정) 
...
-- 상품테이블에서 입력한 상품 직업, 고객정보 직업이랑 동일한지 조회 JOIN써서

-- 기존 고객 직업기준으로 이용 가능한 상품 출력 
